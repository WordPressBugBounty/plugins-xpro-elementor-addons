(function (win) {
    ...
    function getShadowCss(x, y, blur, spread, intensity) {
        return round(x) + "px " + round(y) + "px " + round(blur) + "px " + (spread ? round(spread) + "px " : "") + "rgba(0,0,0," + round(intensity) + ")";
    }
    ...
    function getEventCoordinates(evt, el) {
        ...
        return { x: pos.clientX - el.offsetLeft + window.scrollX, y: pos.clientY - el.offsetTop + window.scrollY };
    }
    ...
    Peel.prototype.setMode = function (mode) {
        if (mode === "book") {
            ...
            this.setOption("backReflection", !1);
            this.setOption("backShadowDistribute", !1);
            this.setOption("bottomShadowDistribute", !1);
    ...
    Peel.prototype.setupDragEvents = function (el) {
        ...
        this.addEvent(el, "mousedown", dragStart.bind(this, !1));
        this.addEvent(el, "touchstart", dragStart.bind(this, !0));
        this.dragEventsSetup = !0;
    };
    ...
    Peel.prototype.addEvent = function (el, type, fn) {
        addEvent(el, type, fn);
        this.events.push({ el: el, type: type, handler: fn });
        return fn;
    };
    ...
    Peel.prototype.setupDropShadow = function (shape, parent) {
        var svg = createSVGElement("svg", parent, { class: prefix("layer") });
    ...
    Peel.prototype.getScaledBox = function (scale) {
        ...
        return [new LineSegment(tl, tr), new LineSegment(tr, br), new LineSegment(br, bl), new LineSegment(bl, tl)];
    };
    ...
    Peel.prototype.setBottomShadow = function (t) {
        ...
        if (this.canSetLinearEffect("bottomShadow", t)) {
            ...
            var midShadowStart = darkShadowStart - this.distributeOrLinear(t, sDistribute, 0.03) * sSize - offset;
            var lightShadowStart = midShadowStart - (0.02 * sSize - offset);
            stops = [getBlackStop(0, 0), getBlackStop(0, lightShadowStart), getBlackStop(lightAlpha, midShadowStart), getBlackStop(lightAlpha, darkShadowStart), getBlackStop(darkAlpha, t)];
        }
    ...
    function SVGClip(el, shape) {
        this.el = el;
        this.shape = SVGClip.createClipPath(el, shape || { type: "polygon" });
        setTransform(this.el, "translate(0px,0px)");
    }
    SVGClip.getDefs = function () {
        if (!this.defs) {
            this.svg = createSVGElement("svg", null, { class: prefix("svg-clip-element") });
    ...
    Circle.prototype.boundingRectContainsPoint = function (p) {
        return p.x >= this.center.x - this.radius && p.x <= this.center.x + this.radius && p.y >= this.center.y - this.radius && p.y <= this.center.y + this.radius;
    };

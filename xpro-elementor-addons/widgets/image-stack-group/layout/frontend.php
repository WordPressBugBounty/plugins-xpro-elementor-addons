...
defined( 'ABSPATH' ) || die();
 ...
 if (empty($settings['images'])) {
    ...
    if (!empty($settings['gradient_border']) && 'yes' === $settings['gradient_border']) {
        ...
        if ($gradient_start && $gradient_end) {
            $gradient_css = "
                ...
                .xpro-image-stacked-item::before {
                    content: '';
                    position: absolute;
                    top: -2px;
                    left: -2px;
    ...
    <div class="xpro-image-stacked-group <?php echo esc_attr($group_layout); ?> hover-<?php echo esc_attr($hover_animation); ?>">
        ...
        foreach ($settings['images'] as $index => $item) :
            ...
            <<?php echo esc_attr($tag); ?>
                <?php echo $this->get_render_attribute_string($item_key); ?>>
